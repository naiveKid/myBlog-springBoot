...
import com.base.util.ChineseToEnglishUtil;
...
public class WebController {
	...
	public void upload(HttpServletRequest request, HttpServletResponse response) {
		...
		String upaloadUrl = request.getSession().getServletContext().getRealPath("/") + "static/upfiles/";// 得到当前工程路径拼接上文件名
		...
		for (MultipartFile file : files.values()) {
			...
			fileName = ChineseToEnglishUtil.converterToSpell(fileName);
			File tagetFile = new File(upaloadUrl + fileName);// 创建文件对象
			while (true) {
				if (tagetFile.exists()) {
					String one = fileName.substring(0, fileName.lastIndexOf(".")) + "1";
					String two = fileName.substring(fileName.lastIndexOf("."));
					fileName = one + two;
					tagetFile = new File(upaloadUrl + fileName);
				} else {
					break;
				}
			}
			...
			try {
				...
				picture.setPictureName("/static/upfiles/" + fileName);
	...
	public ModelAndView downLoad(HttpServletRequest request, HttpServletResponse response, String fileName,
		...
		try {
			// 以流的形式下载文件
			InputStream fis = new BufferedInputStream(new FileInputStream(basepath + filePath));
	...
	public String qrCode(HttpServletRequest request, String info) {
		String uploadUrl = request.getSession().getServletContext().getRealPath("/") + "static/upfiles/";// 得到当前工程路径拼接上文件名
		...
		if (QRCodeUtil.encode(uploadUrl, fileName, info)) {
            return "/static/upfiles/" + fileName + ".png";
        } else {
            return "fail";
        }
	}
